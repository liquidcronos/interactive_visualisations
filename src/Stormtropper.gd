...
var seperation_force = 0
var avoidance_force = 0
var hunting_force   = 0.2
...
func _process(delta):
	...
	var falcon_pos= get_node("/root/Floor Simulation/R2D2").position
	...
	acceleration += process_hunt_falcon(falcon_pos) * hunting_force
...
func process_cohesion(neighbors):
	...
	#P controller towards mass center (K=steer force)
	return steer((vector - position).normalized() * move_speed)
		
func process_hunt_falcon(falcon_pos):
	#P controller towards mass center (K=steer force)
	return steer((falcon_pos - position).normalized() * move_speed)
